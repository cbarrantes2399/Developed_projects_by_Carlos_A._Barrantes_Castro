...
classdef sequential < handle
  ...
  methods
        ...
        for numMB=1:totalBatch; 
          ...
          for i=1:numLayers-1 ## Excluya capa de pérdida 
            if (s.layers{i}.hasState())
              ...
              case "momentum"
                if(isempty(s.v{i}))
                  s.v{i} = s.layers{i}.stateGradient();
                else
                  s.v{i} = s.beta*s.v{i} + (1-s.beta)*s.layers{i}.stateGradient();
                endif
                s.layers{i}.setState(s.layers{i}.state() -
                                     s.alpha*s.v{i});
              ...
              case "rmsprop"
                if(isempty(s.v{i}))
                  ...
                  s.v2{i} = s.v{i}.^2;
                    ...
                    g = s.layers{i}.stateGradient();
                    one = ones(size(s.v2{i}));
                    s.v2{i} = s.v{i}.^2;
                    s.v2{i} = s.beta2*s.v2{i} + (1-s.beta2)*(g.^2);
                    b = pinv(diag((s.v2{i} + s.epsilon * one).^(1/2)));
                    b = sum(b);
                    s.layers{i}.setState(s.layers{i}.state() -
                                      s.alpha*b.*s.layers{i}.stateGradient());
                endif
              case "adam"
                if(isempty(s.v{i}))
                  s.v{i} = s.layers{i}.stateGradient();
    ...
    function losslog=train_pingu(s,X,Y,valSetX=[],valSetY=[])
      ## Entrene el modelo
      ## X: matriz de diseño (datos de entrenamiento en filas)
      ## y: matriz de salida, cada file codificada one-hot
      ## valSetX: set de validación (opcional) (entradas en filas)
